...
impl LinksContainer {
    ...
    pub fn fill_from(&mut self, points: &[PointOffsetType]) {
        self.links.clear();
        self.links.extend_from_slice(points);
        self.correct = 0;
    }
    ...
    pub fn fill_from_sorted_with_heuristic(
        ...
        self.correct = self.links.len() as u32;
    }
    ...
    pub fn connect_with_heuristic(
        ...
        for (order, &link) in self.links.iter().enumerate() {
            items.0.push(Item {
                idx: link,
                score: Cell::new(None),
                order: if order < self.correct as usize {
